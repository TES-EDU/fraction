...
import { ArrowLeft, RefreshCw, User, Share2, Check, Trash2, TrendingUp, TrendingDown, AlertCircle, FileText, Timer, ChevronRight } from 'lucide-react';
...
const formatAnswer = (val: number | string | null) => {
...
function LevelBars({ rows }: { rows: MathResultRow[] }) {
  const byLevel = useMemo(() => {
    ...
    const levels = ['FRACTION_L1', 'FRACTION_L2', 'FRACTION_L3', 'FRACTION_L4'] as const;
    return levels
      .filter(k => map[k])
      .map(k => ({ level: k.replace('FRACTION_L', '단계 '), avg: Math.round(map[k].sum / map[k].total), count: map[k].total }));
  }, [rows]);
...
export default function StudentHistoryScreen({ studentName, onBack, onReportClick }: Props) {
  ...
  const frequentWrong = useMemo(() => {
    const map = new Map<string, { expression: string; correctAnswer: number | string; wrongCount: number; totalSeen: number; unitId: string }>();
    rows.forEach(r => {
    ...
    <div className="bg-sb-surface border border-sb-line rounded-2xl p-5">
      <div className="flex items-center gap-3.5">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[13px] text-sb-muted mt-0.5">TES 분수 응시자</div>
        </div>
        <button
  ...
  return (
    <div className="min-h-screen bg-sb-bg flex flex-col">
      <header className="bg-sb-surface border-b border-sb-line h-14 px-5 flex items-center gap-2.5 shrink-0">
        ...
        <span className="text-sm font-extrabold text-sb-ink -tracking-tight">TES 분수</span>
                  ...
                  return (
                    <div key={i} className="flex items-center gap-2.5 px-3 py-2.5 bg-sb-bg rounded-xl">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-sb-muted">{formatAnswer(w.correctAnswer)}</div>
                      </div>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded tracking-wide ${c.chip}`}>{h.unit_title.startsWith('FRACTION_L') ? h.unit_title.replace('FRACTION_L', '단계 ') : h.unit_title}</span>
                    ...
                    <div className="text-[11px] text-sb-muted flex items-center gap-2">
                      ...
                      <span className="inline-flex items-center gap-0.5"><Timer size={10} /> {formatDuration(h.time_seconds ?? 0)}</span>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-sb-muted mt-0.5">
                  {rows[0] ? `최근 응시 ${new Date(rows[0].created_at ?? '').toLocaleDateString('ko-KR')}` : 'TES 분수 응시자'}
                </div>
              </div>
            </div>
                  ...
                  {frequentWrong.map((w, i) => (
                    <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-sb-wrong-pale/40">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-sb-muted truncate">{formatAnswer(w.correctAnswer)}</div>
                      </div>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            <span className={`text-[10px] font-extrabold px-1.5 py-0.5 rounded ${c.chip}`}>{r.unit_title.startsWith('FRACTION_L') ? r.unit_title.replace('FRACTION_L', '단계 ') : r.unit_title}</span>
