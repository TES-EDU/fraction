...
const STORAGE_KEYS = {
  BEST_SCORE: 'fractiontest_bestScore',
  BEST_COMBO: 'fractiontest_bestCombo',
  RECENT_RESULTS: 'fractiontest_recentResults',
  SOUND_SETTINGS: 'fractiontest_soundSettings',
  SELECTED_MODE: 'fractiontest_selectedMode',
  SELECTED_LEVEL: 'fractiontest_selectedLevel',
  STUDENT_NAME: 'fractiontest_studentName',
  LAST_BRANCH: 'fractiontest_lastBranch',
} as const;
...
export function setCurrentStudent(student: StudentSession): void {
  sessionStorage.setItem('fractiontest_currentStudent', JSON.stringify(student));
}

export function getCurrentStudent(): StudentSession | null {
  const stored = sessionStorage.getItem('fractiontest_currentStudent');
...
export function clearCurrentStudent(): void {
  sessionStorage.removeItem('fractiontest_currentStudent');
}
